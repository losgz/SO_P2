...
int main(int argc, char *argv[]) {
    ...
    if (shmemAttach(shmid, (void **)&sh) == -1) {
        perror(
            "error on mapping the shared region on the process address space");
        return EXIT_FAILURE;
    }
    ...
    if (shmemDettach(sh) == -1) {
        perror("error on unmapping the shared region off the process address "
               "space");
        return EXIT_FAILURE;
        ;
    }
 ...
 *  If there are enough free players and free goalies to form a team, player
 * forms team allowing team members to proceed and waiting for them to
 * acknowledge registration. Otherwise it updates state, waits for the forming
 * teammate to "call" him, saves its team and acknowledges registration. The
...
static int playerConstituteTeam(int id) {
    ...
    // if theres still space in the teams
    if (sh->fSt.playersArrived <= 2 * NUMTEAMPLAYERS) {

        // if theres not enough elements to form a team
        if (sh->fSt.playersFree < NUMTEAMPLAYERS ||
            ...
            for (int i = 0; i < NUMTEAMPLAYERS - 1; i++) {
                ...
                if (semDown(semgid, sh->playerRegistered) == -1) {
                    perror(
                        "error on the up operation for semaphore access (PL)");
                    exit(EXIT_FAILURE);
                }
